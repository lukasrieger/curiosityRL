...
\end{subfigure}
\caption{Selbstständiges Finden von unterschiedlichen Pfaden im zweidimensionalen Raum} \label{img:diayn_ex}
\source{\cite{diversity_eysenbach}}
\end{figure}

Wie bereits mehrfach angemerkt ist das Ziel das eigenständige Erlernen von vielfältigen Fähigkeiten. \cite{diversity_eysenbach} liefert zunächst das simple Beispiel einer zweidimensionalen Navigation in einem leeren Raum (Abbildung \ref{img:diayn_ex}). Hierbei stellen die farbigen Linien unterschiedliche Fähigkeiten dar. Eine Fähigkeit entspricht also im Experiment einem erkundeten Pfad innerhalb des Graphen, dessen mögliche Koordinaten den Zustandsraum beschreiben.
